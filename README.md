...
During the 'pyramidisation', downscaled images are generated by computing an average (in XY) over the previous higher resolution scale (and not by sampling a single pixel). This leads to high SNR of low resolution scaled versions (left: kheops, right: bftools):
...
3. Click `Run`
![img.png](images/batch-command.png)
...
All tests have been performed with bio-formats 9.6.1. The test machine is a 16 core computer from 2018, files are on a SSD drive and written back to the same drive.
...
Kheops can write in parallel since each series is written to a different file. That's why it can outperform the glencoe conversion in this particular case.
...
Files Size | Method | Run time | Effective speed (Mo/s)
--- | --- | --- | ---
9.2 Go | Kheops | 47s | 200 Mo/s
9.2 Go | Glencoe | 142s | 66 Mo/s
9.2 Go | Bftools | 270s | 34.9 Mo/s
